  ...
  <body>
    ...
    <main id="main" class="main">
      ...
      <div class="container shadow p-4">
        <form action="${pageContext.request.contextPath}/admin/danhsachkhoa/sua" method="post">
          <input type="hidden" name="MaKhoa" value="${khoa.MaKhoa}" />
          <div class="row">
            <div class="form-group col-md-6 mb-3">
              <label>Mã khoa</label>
              <input type="text" class="form-control" name="MaKhoa" value="${khoa.MaKhoa}" readonly />
            </div>
            <div class="form-group col-md-6 mb-3">
            ...
            <div class="form-group col-md-6 mb-3">
              ...
              <select name="MaVien" class="form-select">
                ...
                <c:forEach var="vien" items="${danhSachVien}">
                  <option value="${vien.MaVien}" <c:if test="${khoa.MaVien == vien.MaVien}">selected</c:if>>${vien.TenVien}</option>
                </c:forEach>
              </select>
            ...
            <div class="form-group col-md-6 mb-3">
              ...
              <input type="text" class="form-control" name="TenTruongKhoa" value="${khoa.TenTruongKhoa}" />
              <c:if test="${not empty loiTenTruongKhoa}"><small class="text-danger">${loiTenTruongKhoa}</small></c:if>
            </div>
            <div class="form-group col-md-6 mb-3">
              <label>Số điện thoại</label>
              <input type="text" class="form-control" name="SoDienThoaiKhoa" value="${khoa.SoDienThoaiKhoa}" />
              <c:if test="${not empty loiSoDienThoaiKhoa}"><small class="text-danger">${loiSoDienThoaiKhoa}</small></c:if>
            </div>
            <div class="form-group col-md-6 mb-3">
              <label>Email</label>
              <input type="text" class="form-control" name="EmailKhoa" value="${khoa.EmailKhoa}" />
              <c:if test="${not empty loiEmailKhoa}"><small class="text-danger">${loiEmailKhoa}</small></c:if>
            </div>
            <div class="form-group col-md-6 mb-3">
              <label>Ngày thành lập</label>
              <input type="date" class="form-control" name="NgayThanhLapKhoa" value="${khoa.NgayThanhLapKhoa}" />
              <c:if test="${not empty loiNgayThanhLapKhoa}"><small class="text-danger">${loiNgayThanhLapKhoa}</small></c:if>
            </div>
            <div class="form-group col-md-6 mb-3">
              <label>Mô tả</label>
              <textarea class="form-control" name="MoTaKhoa" rows="3">${khoa.MoTaKhoa}</textarea>
            </div>
          </div>
